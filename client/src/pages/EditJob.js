...
function EditJob({ match }) {
  ...
  const jobToBeEdited = jobs.find((job) => job._id === id);
  console.log(jobToBeEdited);
  return (
    <div>
      <DefaultLayout>
        <Tabs defaultActiveKey="0" activeKey={activeTab}>
          <TabPane tab="Job Information" key="0">
            <Form
              layout="vertical"
              onFinish={onJobInformationFinish}
              initialValues={jobToBeEdited}
            >
              <Row gutter={16}>
                <Col lg={8} sm={24}>
                  <Form.Item
